...
# 第15章 存储管理
程序运行时，会将各类数据分类组织存放在内存空间中，并常常需要动态地分配和撤销内存空间。
例如，对动态链接表中的节点进行插入和删除，就要对内存进行管理。

## 15.1 内存组织方式
计算机程序的机器语言指令和数据都存储在同一个逻辑内存空间里。
...
### 16.2.1 套接字概述
所谓套接字，实际上是一个指向传输提供者的句柄。在WinSock中，就是通过操作该句柄来实现网络通信和管理的。根据性质和作用的不同，套接字可以分为原始套接字、流式套接字和数据包套接字3种。
+ 原始套接字：是在WinSock2规范中提出，它能够使程序开发人员对底层的网络传输机制进行控制，在原始套接字下接收的数据中包含IP头。
...
基于TCP，面向连接的socket编程的服务器端程序流程如下：
(1) 创建套接字socket。
...
**注意**
在服务器端要建立套接字绑定到指定的主机IP和端口上等待客户的请求，但是对于客户端来说，当发起连接请求并被接受后，在服务器端就保持了该客户端的IP地址和端口号的信息。对于利用返回的套接字进行与客户端的通信。
...
**注意**
基于UDP的套接字编程时，仍然需要使用bind进行绑定。虽然面向无连接的socket编程无须建立连接，但为了完成通信，首先应启动接受端，等待接收发送端发来的数据，这样接收端就必须告诉它自己的地址和端口。因此，必须调用bind函数将套接字绑定到一个本地地址和端口上。

基于UDP的套接字编程时，利用的是sendto和recvfrom两个函数实现数据的发送和接收；基于TCP的套接字编程时，发送数据使用的是send函数，接收数据使用的是recv函数。

## 16.3 套接字函数
前面介绍的使用套接字编写程序的流程，本节介绍使用套接字编程时用到的函数。
### 16.3.1 套接字常用函数介绍
1. WSAStartup函数
